...
$("#addBtn").on("click", function () {
    ...
    $.ajax({
        url:  baseUrl + "admin/manage_rndpartner/new",
        data: postData,
        type: "POST",
...
$("#yesApprove").on("click", function () {
    ...
    $.ajax({
        url: baseUrl + "admin/manage_rndpartner/trash",
        data: postData,
        type: "POST",
        success: function (output) {
            var data = output.trim().split("::");
...
//When No has been Selected on Approval Modal, Just Un-Trash the Selected Data.
    ...
    $("#permanentDelete").on("click", function () {
        ...
        $.ajax({
            url: baseUrl + "admin/manage_rndpartner/delete",
            data: postData,
            type: "POST",
            success: function (output) {
                var data = output.trim().split("::");
    ...
    $("#updateBtn").on("click", function () {
        ...
        $.ajax({
            url: baseUrl + "admin/manage_rndpartner/update",
            data: postData,
            type: "POST",
            success: function (output) {
                var data = output.trim().split("::");
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                ...
                $.ajax({
                    crossOrigin: true,
                    type: 'POST',
                    url: baseUrl + "admin/manage_rndpartner/updateLogo",
                    data: formData,
                    processData: false,
                    contentType: false
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                ...
                $.ajax({
                    crossOrigin: true,
                    type: 'POST',
                    url: baseUrl + "admin/manage_rndpartner/updateImage",
                    data: formData,
                    processData: false,
                    contentType: false
                }).done(function (response) {
