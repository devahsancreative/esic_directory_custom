...
class GrantRecipients extends MY_Controller {
    ...
    public function ManageGrantRecipients($param=NULL){
        ...
        if($param === 'listing'){
            ...
            $returnedData = $this->Common_model->select_fields_joined_DT($selectData,'esic_grantrecipients','','','','','',$addColumns);
        ...
        if($param === 'new'){
            ...
            if(empty($Name)){
                echo "FAIL::Grant Recipients Name is Required.";
                return NULL;
            }elseif(!is_string($Name)){
                echo "FAIL::Grant Recipients Name must be a valid string.";
                return NULL;
            }
            ...
            $insertResult = $this->Common_model->insert_record('esic_grantrecipients',$insertData);
            if($insertResult){
        ...
        if($param === 'trash'){
            ...
            $returnedData = $this->Common_model->update('esic_grantrecipients',$whereUpdate,$updateData);
        ...
        if($param === 'update'){
            ...
            if(empty($NameUpdated)){
                echo "FAIL::Grant Recipients name is a required field::error::Required!!";
                return NULL;
            }
            ...
            $updateResult = $this->Common_model->update('esic_grantrecipients',$whereUpdate,$updateData);
        ...
        if($param === 'delete'){
            ...
            if($value == 'delete'){
                ...
                $returnedData = $this->Common_model->delete('esic_grantrecipients',$whereUpdate);
        ...
        if($param === 'updateLogo'){
			...
			$uploadPath 		= './pictures/logos/grantrecipients';
            $uploadDirectory 	= './pictures/logos/grantrecipients';
            $uploadDBPath 		= 'pictures/logos/grantrecipients';
            ...
            if(isset($_FILES['logo']['name'])){
                ...
                if(!in_array(strtolower($ext),$allowedExt)){
                    ...
                    $FileName = "grantRecipientsLogo".$ID."_".time().".".$ext;
            ...
            if(empty($ID)){
            	...
            	$where = array( 'id' => $ID );
            	$returnedData = $this->Common_model->select_fields_where('esic_grantrecipients',$selectData, $where, false, '', '', '','','',false);
            	$logo = $returnedData[0]->logo;
            if(!empty($logo) && is_file(FCPATH.'/'.$logo)){
                unlink('./'.$logo);
            }
            	$resultUpdate = $this->Common_model->update('esic_grantrecipients',$where,$insertDataArray);
        ...
        $this->show_admin('admin/configuration/grantrecipients');
        return NULL;
    }
}