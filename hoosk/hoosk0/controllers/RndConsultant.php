...
class RndConsultant extends MY_Controller {
    ...
    public function ManageRndConsultant($param=NULL){
        ...
        if($param === 'listing'){
            ...
            $returnedData = $this->Common_model->select_fields_joined_DT($selectData,'esic_rndconsultant','','','','','',$addColumns);
        ...
        if($param === 'new'){
            ...
            if(empty($Name)){
                echo "FAIL::Rnd Consultant Name is Required.";
                return NULL;
            }elseif(!is_string($Name)){
                echo "FAIL::Rnd Consultant Name must be a valid string.";
                return NULL;
            }
            ...
            $insertResult = $this->Common_model->insert_record('esic_rndconsultant',$insertData);
            if($insertResult){
        ...
        if($param === 'trash'){
            ...
            $returnedData = $this->Common_model->update('esic_rndconsultant',$whereUpdate,$updateData);
        ...
        if($param === 'update'){
            ...
            if(empty($NameUpdated)){
                echo "FAIL::Rnd Consultant name is a required field::error::Required!!";
                return NULL;
            }
            ...
            $updateResult = $this->Common_model->update('esic_rndconsultant',$whereUpdate,$updateData);
        ...
        if($param === 'delete'){
            ...
            if($value == 'delete'){
                ...
                $returnedData = $this->Common_model->delete('esic_rndconsultant',$whereUpdate);
        ...
        if($param === 'updateLogo'){
			...
			$uploadPath 		= './pictures/logos/rndconsultant';
            $uploadDirectory 	= './pictures/logos/rndconsultant';
            $uploadDBPath 		= 'pictures/logos/rndconsultant';
            ...
            if(isset($_FILES['logo']['name'])){
                ...
                if(!in_array(strtolower($ext),$allowedExt)){
                    ...
                    $FileName = "rndConsultantLogo".$ID."_".time().".".$ext;
            ...
            if(empty($ID)){
            	...
            	$where = array( 'id' => $ID );
            	$returnedData = $this->Common_model->select_fields_where('esic_rndconsultant',$selectData, $where, false, '', '', '','','',false);
            	$logo = $returnedData[0]->logo;
            if(!empty($logo) && is_file(FCPATH.'/'.$logo)){
                unlink('./'.$logo);
            }
            	$resultUpdate = $this->Common_model->update('esic_rndconsultant',$where,$insertDataArray);
        ...
        $this->show_admin('admin/configuration/rndconsultant');
        return NULL;
    }
}